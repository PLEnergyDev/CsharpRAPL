...
public class BenchmarkCollectorTest {
	...
	public void TrySetFieldTest01() {
		...
		BenchmarkCollector.TrySetField(typeof(DummyBenchmarks), "TestField1", 10);
	...
	public void TrySetFieldTest02() {
		var exception = Assert.Throws<NotSupportedException>(() =>
			BenchmarkCollector.TrySetField(typeof(DummyBenchmarks), "TestField2", 10));
		Assert.AreEqual("Your TestField2 field must be static.", exception?.Message);
	}

	[Test]
	public void TrySetFieldTest03() {
		Assert.False(BenchmarkCollector.TrySetField(typeof(DummyBenchmarks), "TestField3", 10));
	}
}